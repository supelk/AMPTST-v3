...
class Model(nn.Module):
    ...
    def forward(self, x_enc, x_mark_enc,dec_inp, batch_y_mark):
        ...
        seasonal_init, trend_init = seasonal_init, trend_init.permute(0, 2, 1)
        trend_output = self.Linear_Trend(trend_init)
        x_enc, x_mark_enc = self.__multi_scale_process_inputs(seasonal_init, x_mark_enc)
        x_list = []
        x_mark_list = []
        if x_mark_enc is not None:
        ...
        out_list = self.season(x_list)  # [B*N,T,dm] or [B,T,dm]
        oc_list = []
        ...
        x = output + trend_output.permute(0, 2, 1)
        return x