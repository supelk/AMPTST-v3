...
class AMPTST(nn.Module):
    def __init__(self, configs):
        ...
        self.pf = configs.pf
        self.period_scorer = nn.Linear(configs.d_model,1)
    ...
    def forward(self, x_list):  # [B,T,dm]
        ...
        for i,x in enumerate(x_list):
            ...
            for k in range(self.k):
                ...
                # ============ fusion: multiplicative interaction =============
                out = (block_season * block_trend).reshape(B, -1, N)
                res.append(out[:,:T,:])

            res = torch.stack(res, dim=-1)
            score = self.period_scorer(  # 输入: [B, T, N, k]
                res.permute(0, 3, 1, 2)  # -> [B, k, T, N]
            )  # 输出: [B, k, T, 1]
            score = score.mean(dim=2)  # -> [B, k, 1]
            score = score.squeeze(-1)  # -> [B, k]
            # ======= (2) sigmoid-normalized weights =======
            weight = torch.sigmoid(score)
            weight = weight / (weight.sum(dim=1, keepdim=True) + 1e-6)  # normalize

            # reshape to broadcast
            weight = weight.unsqueeze(1).unsqueeze(1)  # [B,1,1,k]

            # ======= (3) weighted sum fusion =======
            res = (res * weight).sum(dim=-1)  # [B, T, N]
            # residual connection
            res = res + x
            res_list.append(res)
        return res_list
