...
class FlattenHead(nn.Module):
    ...
    def forward(self, x):  # x: [bs x nvars x d_model x patch_num]
        ...
        x = self.dropout(x)
        return x
class MSP_MLP(nn.Module):
    def __init__(self, configs):
        super(MSP_MLP, self).__init__()
        ...
        self.pf = configs.pf
        self.Multi_MLP_inter = nn.ModuleList(
            [
                Encoder(
                    [
                        EncoderLayer(
        ...
        self.Multi_MLP_intra = nn.ModuleList(
            [
                Encoder(
                    [
                        EncoderLayer(
    ...
    def forward(self, x_list):  # [B,T,dm]
        ...
        for i,x in enumerate(x_list):
            ...
            B, T, N = x.size()
            # 分patch操作，[B,N,P_num,P_len]
            y = x.unfold(dimension=-1, size=self.patch_len, step=self.stride)
            res = []
            # seasons =[]
            # trends =[]
                # padding
